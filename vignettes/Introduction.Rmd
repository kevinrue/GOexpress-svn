...
```{r bestFeature.rf}
bestFeature.rf <- head(rownames(
    randomForest::importance(rf)[order(
        randomForest::importance(rf)[,"MeanDecreaseGini"],
        decreasing = TRUE),
    ]), 1)
```
...
ggplot(ggfy(
    ...
    geom_line(aes(Time, exprs, colour = Infection)) +
